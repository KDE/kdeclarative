...
QString SettingStateProxy::itemName() const
{
    return m_itemName;
}

void SettingStateProxy::setItemName(const QString &itemName)
{
...
void SettingStateProxy::resetToDefault()
{
    const auto item = m_configObject ? m_configObject->findItem(m_itemName) : nullptr;
    if (item) {
        item->setDefault();
    }
}
...
void SettingStateProxy::connectSetting()
{
    ...
    if (!itemHasSignals) {
        qWarning() << "Attempting to use SettingStateProxy with a non signalling item:" << m_itemName;
        return;
    }

    const auto propertyName = [this] {
        auto name = m_itemName;
